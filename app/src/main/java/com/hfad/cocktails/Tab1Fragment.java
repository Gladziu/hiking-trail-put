...
public class Tab1Fragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        String[] trailNames = new String[Trail.summerTrails.length];
        for (int i = 0; i < trailNames.length; i++) {
            trailNames[i] = Trail.summerTrails[i].getName();
        }
        int[] trailImages = new int[Trail.summerTrails.length];
        for (int i = 0; i < trailImages.length; i++) {
            trailImages[i] = Trail.summerTrails[i].getImageResourceId();
        }
        ...
        adapter.setListener(new CaptionedImagesAdapter.Listener() {
            public void onClick(int position) {
                ...
                intent.putExtra(TrailDetailActivity.EXTRA_TRAIL_CATEGORY, "summer");
